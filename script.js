...
function adaptInputColor() {
  colorLabels.forEach((label) => {
    ...
    const yik = (red * 299 + green * 587 + blue * 114) / 1000;
...
const rangerInput = document.querySelector(".inp-rang");
rangerInput.addEventListener("input", handleInclination);

function handleInclination() {
  gradientData.angle = rangerInput.value;
  rangeLabelValue.textContent = `${gradientData.angle}
`;
  populateUI();
}
